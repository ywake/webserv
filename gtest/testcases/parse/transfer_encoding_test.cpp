...
{
	EXPECT_TRUE(http::headers::IsValidTransferEncoding(HeaderSection("")));
	EXPECT_TRUE(http::headers::IsValidTransferEncoding(HeaderSection("Transfer-Encoding:chunked\r\n"
	)));
	EXPECT_TRUE(http::headers::IsValidTransferEncoding(HeaderSection("Transfer-Encoding:gzip\r\n"
	...
	EXPECT_THROW(
		http::headers::IsValidTransferEncoding(HeaderSection("Transfer-Encoding: unknown\r\n")),
		Error
	);
	EXPECT_THROW(
		http::headers::IsValidTransferEncoding(HeaderSection("Transfer-Encoding: trailers\r\n")),
		Error
	);
	EXPECT_THROW(
		http::headers::IsValidTransferEncoding(HeaderSection("Transfer-Encoding: gzip\r\n")), Error
	);
}
